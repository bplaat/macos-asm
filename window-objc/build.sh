...
if [[ $1 = "release" ]]; then
    clang $CFLAGS -Os --target=arm64-macos bassietest.m -framework Cocoa -o BassieTest-arm64
    clang $CFLAGS -Os --target=x86_64-macos bassietest.m -framework Cocoa -o BassieTest-x86_64
    ...
    clang $CFLAGS bassietest.m -framework Cocoa -o BassieTest.app/Contents/MacOS/BassieTest
fi
